...
async def main():
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get("https://deckofcardsapi.com/api/deck/zsj39xrshrom/draw/?count=1") as response:  
            ...
            image_url = json_data["cards"][0]["image"]
            extension = image_url.split(".")[-1]  #  расиширение файла
            print(f"Image URL: {image_url}")

        async with session.get(image_url) as response:
            total_bytes, wrote_bytes = response.headers["Content-Length"], 0
            print("Open file...")
            async with async_open(f"random_card.{extension}", "wb") as f:
